...
export async function sendPasswordResetEmail(email: string, token: string) {
  ...
  await getMailgunClient().messages.create(DOMAIN, {
    from: FROM,
    to: [email],
    subject: "Reset your Agency Advisory password",
    html: `
      <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; padding: 32px;">
        <h2 style="color: #00C4CC; margin-bottom: 16px;">Password Reset</h2>
        <p style="color: #333; line-height: 1.6;">
          You requested a password reset for your Agency Advisory account. Click the button below to set a new password.
        </p>
...
export async function sendWelcomeEmail(email: string, name: string) {
  await getMailgunClient().messages.create(DOMAIN, {
    from: FROM,
    to: [email],
    subject: "Welcome to Agency Advisory",
    html: `
      <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; padding: 32px;">
        <h2 style="color: #00C4CC; margin-bottom: 16px;">Welcome, ${name || "there"}!</h2>
        <p style="color: #333; line-height: 1.6;">
          Your Agency Advisory account is ready. You now have access to a full suite of AI tools designed to help you run, grow, and scale your automation agency.
        </p>
